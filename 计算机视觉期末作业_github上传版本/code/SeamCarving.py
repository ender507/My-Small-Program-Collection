...
def imSave(pic, num, save_path, show):
    # 展示1s的图片
    '''
    if show: 
        ...
        if save_frames != -1:   # 保存gif的一帧
            if mode == 1:
                imSave(img, str(save_frames)+'_'+str(frame),
                       "output\\SeamCarving\\"+str(save_frames)+"_frames\\", False)
            else:
...
if __name__ == '__main__':
    ...
    for i in range(57, 1000, 100):
        ...
        # 计算输出图片的大小
        size = img.size
        front_ground = np.sum(np.array(ground) == 1)
        rate = 1 - (2*front_ground / (size[0]*size[1]))
        print(rate)
        new_size = (int(size[0]*(rate**pic_rate[i//100])), int(size[1]*(rate**(1-pic_rate[i//100]))))
        # 是否保存每一帧，用于生成gif
        if i==57 or i==557:
            save_frames = i
        else:
            save_frames = -1
        # 先减小宽度
        img, ground = widthCarve(img, ground, new_size[0], save_frames, 1)
        # 再减小高度
        img, ground = heightCarve(img, ground, new_size[1], save_frames)
        imSave(img, i, "output\\SeamCarving\\", True)
