...
def dispP1_2():
    window = util.createWindow()
    tk.Label(window, text='测试题',font=('Arial', 30)).pack()
    tk.Label(window, text='请依据图片完成下面的测试题：',font=('Arial', 15)).pack()
    photo = Image.open("src\\test.png")
    photo = photo.resize((500,170)) 
    photo = ImageTk.PhotoImage(photo)
    imgLabel = tk.Label(window,image=photo).pack()
    ans = [tk.StringVar()for i in range(5)]
    tk.Label(window, text='1. 如果不作出改变，物资将送往哪个地区？',font=('Arial', 15)).pack()
    tk.Radiobutton(window, text='上方地区',variable=ans[0], value='A').pack()
    tk.Radiobutton(window, text='下方地区',variable=ans[0], value='B').pack()
    tk.Label(window, text='2. 小人的个数表示？',font=('Arial', 15)).pack()
    ...
    tk.Label(window, text='3. 小人身上的红色数字代表？',font=('Arial', 15)).pack()
    ...
    tk.Label(window, text='4. 就平均值（红色条形图）而言，哪个地区的更大？',font=('Arial', 15)).pack()
    tk.Radiobutton(window, text='上方地区',variable=ans[3], value='A').pack()
    tk.Radiobutton(window, text='下方地区',variable=ans[3], value='B').pack()
    tk.Label(window, text='4. 就病情总值（红色条形图）而言，哪个地区的更大？',font=('Arial', 15)).pack()
